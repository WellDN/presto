...
(   Divide
    ...
    | GreaterThan
    | LessThan )) ->
          failwith "Internal error: not a binary assembly instructions"
            [@coverage off]
...
let convert_cond_code = function
    ...
    | _ -> failwith "Internal error: not a conditional code" [@coverage off]
