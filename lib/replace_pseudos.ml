...
type replacement_state = {
    ...
    offset_map : (string, int) Map.t; (* map from pseudoregister to stack slots *)
    }

let replace_operand state = function
    (* if it's pseudoregister, replace it with a stack slot *)
    | Pseudo s -> (
        match Map.find_opt s state.offset_map with
        (* We've haven't already assigned it a stack slot;
...
let replace_pseudos_in_instruction state = function
    ...
    | SetCC (code, op) ->
            let state, new_op = replace_operand state op in
            (state, SetCC (code, new_op))
    | (Ret | Cdq | Label _ | JmpCC _ | Jmp _) as other -> (state, other)
    | AllocateStack _ ->
           failwith 
           "Internal error: AllocateStack should't be present at this point"
           [@coverage off]
           
let replace_pseudos_in_function (Function { name; instructions }) =
    let init_state = { current_offset = 0; offset_map = Map.empty } in
